...
map<string, StructSet> NameToStruct;
...
string getOrigName(StructType * ST) {
	string name = ST->getStructName();
...
bool hasName(StructType * ST) {
	string name = ST->getStructName();
	if(!ST->hasName())
		return false;
	if(name.size() > 10 && name.substr(0, 10) == "union.anon")
		return false;
	return true;
}

void ClusterTypesByName(Module * module) {
	vector<StructType *> structVect = module->getIdentifiedStructTypes();
	int no_name = 0;
	for(unsigned i = 0; i < structVect.size(); i++) {
		StructType * ST = structVect[i];
		if(!hasName(ST)) {
			no_name++;
			continue;
		}
		NameToStruct[getOrigName(ST)].insert(ST);
	}
}

StructSet StructsWithSameName(StructType * ST) {
	StructSet structs;
	structs.insert(ST);
	if(!hasName(ST))
		return structs;
	string name = getOrigName(ST);
	return NameToStruct[name];
}

void TypeAliasingInfo(Module * module) {
	// HandleGlobals(module);
	// HandleInsts(module);
	ClusterTypesByName(module);
}

bool SimilarName(StructType * ST1, StructType * ST2) {
	if(!ST1->hasName() || !ST2->hasName())
		return false;
	string name1 = getOrigName(ST1);
	string name2 = getOrigName(ST2);
	return name1 == name2;
}

bool SameStruct(StructType * ST1, StructType * ST2) {
	// if(SimilarName(ST1, ST2)) return true;
	// if(!findInMap(StructToLocation, ST1)) return false;
	// if(!findInMap(StructToLocation, ST2)) return false;
	// return StructToLocation[ST1] == StructToLocation[ST2];
	return SimilarName(ST1, ST2);
}