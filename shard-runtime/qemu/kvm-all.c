...
void mmap_texts(char * ram, uint64_t size) {
    ...
    ram_oos = mmap(NULL, text_section_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd3, 0);
    ram_ud2 = mmap(NULL, text_section_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd4, 0);

    char buffer[100];
    memcpy(buffer, ram_is + (0xffffffff815b4511 - 0xffffffff81000000), 20);
    for(int i = 0; i < 20; i++) {
        printf("%x ", buffer[i]);
    }
    printf("\n");
    ...
    original_addr = (uint64_t) ram + text_section_offset;
    is_addr = (uint64_t) ram_is;
    oos_addr = (uint64_t) ram_oos;
    ud2_addr = (uint64_t) ram_ud2;
    fd_original = fd1;
    fd_is = fd2;
    fd_oos = fd3;
    fd_ud2 = fd4;
...
void dt_initialize(struct kvm_userspace_memory_region mem) {
    ...
    printf("%ld\n", write(fd, (char *) &fd_is, 4));
    printf("%ld\n", write(fd, (char *) &fd_oos, 4));
    printf("%ld\n", write(fd, (char *) &fd_original, 4));
    printf("%ld\n", write(fd, (char *) &fd_ud2, 4));

    printf("%ld\n", write(fd, (char *) &is_addr, 8));
    printf("%ld\n", write(fd, (char *) &oos_addr, 8));
    printf("%ld\n", write(fd, (char *) &original_addr, 8));
...
static int kvm_set_user_memory_region(KVMMemoryListener *kml, KVMSlot *slot, bool new)
{
    ...
    if(mem.guest_phys_addr == 0x100000 && !initialized) {
        dt_initialize(mem);
    } 
    // if(mem.guest_phys_addr == 0x100000) {
    //     mem.userspace_addr = original_addr;
    //     slot->ram = (void *) original_addr;
    // }
