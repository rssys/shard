...
static void vmx_vcpu_reset(struct kvm_vcpu *vcpu, bool init_event)
{
	...
	if (!init_event) {
		...
		#ifndef PROFILING
		vmcs_write64(GUEST_IA32_DEBUGCTL, 1); /* SHARD : Updated to record lbr on guest */
		#else
		vmcs_write64(GUEST_IA32_DEBUGCTL, 0); /* SHARD : Updated to record lbr on guest */
		#endif
	}
...
static void vmx_inject_irq(struct kvm_vcpu *vcpu)
{
	...
	// write_to_log_sprintf("Injecting IRQ %d\n", irq);
	// dt_handle_exit(vcpu);
	// dt_change_scope(vcpu, fdget(fd_original).file, original_us_addr, -1);
...
bool first_time_log_irq_enter = true;
static int handle_exception(struct kvm_vcpu *vcpu)
{
	struct vcpu_vmx *vmx = to_vmx(vcpu);
	struct kvm_run *kvm_run = vcpu->run;
	u32 intr_info, ex_no, error_code;
	unsigned long cr2, rip, dr6, instp;
	u32 vect_info;
	enum emulation_result er;
	vect_info = vmx->idt_vectoring_info;
	intr_info = vmx->exit_intr_info;
	// if(vmcs_readl(GUEST_RIP) == log_fn_addr) {
	// 	if(!fd_log) fd_log = dt_init_log(dt_fpath_kvm_log);
	// 	dt_handle_logging(vcpu);
	//     kvm_register_write(vcpu, VCPU_REGS_RIP, vmcs_readl(GUEST_RIP) + 2);
	//     return 1;
	// } else {
	// 	dt_printk("%lx handle_exception\n", vmcs_readl(GUEST_RIP));
	// }

	if (is_machine_check(intr_info)) {
		// dt_printk("is_machine_check %lx\n", vmcs_readl(GUEST_RIP));
	...
	if (is_invalid_opcode(intr_info)) {
		instp = vmcs_readl(GUEST_RIP);
		// if(instp == set_tracking_2_addr) {
		// 	handle_st2(vcpu);
		// 	return 1;
		// }
		// if(instp == do_fork_addr) {
		// 	handle_do_fork(vcpu);
		// 	return 1;
		// }
		...
		/* SHARD : entry point for SHARD */
	 	if(dt_handle_exception(vcpu, instp, vmcs_readl(GUEST_RSP))) {
			// dt_printk("is_invalid_opcode %lx\n", vmcs_readl(GUEST_RIP));
			// dt_printk("*****************************************\n");
			return 1;
		} else {
			write_to_log_sprintf(fd_log, "is_invalid_opcode %lx\n", vmcs_readl(GUEST_RIP));
			write_to_log_sprintf(fd_log, "*****************************************\n");
			dt_printk("is_invalid_opcode %lx\n", vmcs_readl(GUEST_RIP));
			dt_printk("*****************************************\n");
		}
	...
	if (intr_info & INTR_INFO_DELIVER_CODE_MASK) {
		// dt_printk("INTR_INFO_DELIVER_CODE_MASK %lx\n", vmcs_readl(GUEST_RIP));
...
static int handle_io(struct kvm_vcpu *vcpu)
{
	...
	// unsigned long instp = vmcs_readl(GUEST_RIP);
...
static int handle_ept_violation(struct kvm_vcpu *vcpu)
{
	...
	gfn = gpa >> PAGE_SHIFT;
	/*write_to_log_sprintf(fd_log, "text section page_fault %lld ", gfn);
	if(exit_qualification & EPT_VIOLATION_ACC_READ) {
		write_to_log_sprintf(fd_log, "read\n");
	}
	if(exit_qualification & EPT_VIOLATION_ACC_WRITE) {
		write_to_log_sprintf(fd_log, "write\n");
	}
	if(exit_qualification & EPT_VIOLATION_ACC_INSTR) {
		write_to_log_sprintf(fd_log, "instr\n");
	}
	if((exit_qualification &
		       (EPT_VIOLATION_READABLE | EPT_VIOLATION_WRITABLE |
			EPT_VIOLATION_EXECUTABLE))) {
		write_to_log_sprintf(fd_log, "present\n");
	}*/
...
static void __noclone vmx_vcpu_run(struct kvm_vcpu *vcpu)
{
	...
	if(HAS_HANDLED) {
		#ifdef TIME_IT
    	END_TIMER_WI(total_start_t, total_time);
		#endif
	}
	#ifndef PROFILING
	enable_lbr();
	...
	asm(
		...
		#endif
		#ifdef TIME_IT
		START_TIMER_WI(total_start_t);
		#endif
		if(is_timing) {
			num_guest_exits++;
			// if(num_guest_exits % 2000 == 0)
			// 	print_numbers();
		}
	/* Eliminate branch target predictions from guest mode */
	// dt_printk("instp is %lx", vmcs_readl(GUEST_RIP));
	vmexit_fill_RSB();
