...
#define IS_STACK_FUNC(F) (F->getName() == "ss_entry" || F->getName() == "ss_exit")
#define IS_SC_FUNC(F) (F->getName() == "sc_entry" || F->getName() == "sc_exit" || F->getName() == "log_fn")
#define IS_CFI_FUNC(F) (F->getName() == "check_ci")
...
bool llvm::InlineFunction(CallSite CS, InlineFunctionInfo &IFI,
  ...
  for(f_it = CalledFunc->begin(); f_it != CalledFunc->end(); f_it++) {
    BasicBlock * BB = &*f_it;
    for(b_it = BB->begin(); b_it != BB->end(); b_it++) {
      Instruction * I = &*b_it;
      CallInst * CI = dyn_cast<CallInst>(I);
      if(!CI || !CI->getCalledFunction()) continue;
      if(IS_STACK_FUNC(CI->getCalledFunction()) || IS_SC_FUNC(CI->getCalledFunction())) {
        to_remove.push_back(CI);        
      }
      if(IS_CFI_FUNC(CI->getCalledFunction())) {
        if(!TheCall->getFunction()) continue;
        if(TheCall->getFunction()->getSection() != "") {
          // errs() << "inlining " << CalledFunc->getName() << " into " << TheCall->getFunction()->getName() << "\n";
          to_remove.push_back(CI);
        }
      }
    }
  ...
  { // Scope to destroy VMap after cloning.
    ...
    for(unsigned i = 0; i < to_remove.size(); i++) {
      if(VMap.find(to_remove[i]) == VMap.end()) {
        // errs() << *to_remove[i] << "\n";
        continue;
      }
      // errs() << *VMap[to_remove[i]] << " " << CalledFunc->getName() << "\n";
      Instruction * I = dyn_cast<Instruction>(VMap[to_remove[i]]);
      I->eraseFromParent();
    }
  ...
  if (Returns.size() == 1 && std::distance(FirstNewBlock, Caller->end()) == 1) {
    ...
    // errs() << "__Inline__ : " << Caller->getName() << " " << CalledFunc->getName() << "\n";
    return true;
  }
